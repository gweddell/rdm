...
%token PREPROCESSOR PROP SCHEMA INVOKE 
...
PostfixExpr
   ...
   | INVOKE PostfixExpr '(' ')'
      { printf("TransCall 1 "); }
   | INVOKE PostfixExpr '(' ')' IN Identifier 
      { printf("TransCallWSchema 2 "); }
   | INVOKE PostfixExpr '(' ArgumentExprList ')'
      { printf("TransCallP 2 "); }
   | INVOKE PostfixExpr '(' ArgumentExprList ')' IN Identifier
      { printf("TransCallPWSchema 3 "); }
...
SelectionStatement
   ...
   | SWITCH '(' Expr ')' Statement
      { printf("Switch 2 "); }
   | IF QueryCallSchema Statement
      { printf("IfQuery 2 "); }
   | IF IdentifierList IN QueryCallSchema Statement
      { printf("IfQueryWSelect 3 "); }
   | IF QueryCallSchema Statement ELSE Statement
      { printf("IfElseQuery 3 "); }
   | IF IdentifierList IN QueryCallSchema Statement ELSE Statement
      { printf("IfElseQueryWSelect 4 "); }
   ;

QueryCallSchema
   : QueryCall
      { printf("nil QueryCall 2 "); }
   | QueryCall IN Identifier
      { printf("QueryCallWSchema 2 "); }
   ;

QueryCall
   : Identifier
      { printf("nil ParamQueryCall 2 "); }
   | Identifier '(' ArgumentExprList ')'
      { printf("ParamQueryCall 2 "); }
   ;

IterationStatement
   : WHILE '(' Expr ')' Statement
      { printf("While 2 "); }
   | WHILE QueryCallSchema Statement
      { printf("WhileQuery 2 "); }
   | WHILE IdentifierList IN QueryCallSchema Statement
      { printf("WhileQueryWSelect 3 "); }
   ...
   | FOR '(' Expr ';' Expr ';' Expr ')' Statement
      { printf("ForWISF 4 "); }
   | FOR QueryCallSchema Statement
      { printf("ForQuery 2 "); }
   | FOR IdentifierList IN QueryCallSchema Statement
      { printf("ForQueryWSelect 3 "); }
   ;
...
ExternalDefinition
   ...
   | Declaration
   | PREPROCESSOR
      { printf("\"%s\" Preprocessor 1 ", yytext); }
   | SCHEMA Identifier InitDeclaratorList ';'
      { printf("SchemaDecl 2 "); }
   ;
